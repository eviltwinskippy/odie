...
set fout   [open [file join $path vexpr.c] w]
...
int DLLEXPORT VecExpr_ObjCmd _ANSI_ARGS_((ClientData dummy, Tcl_Interp *tinterp, int objc, Tcl_Obj *CONST objv[]));
...
puts $fout {
int DLLEXPORT VecExpr_ObjCmd(dummy, tinterp, objc, objv) 
...
int DLLEXPORT Vexpr_Init( Tcl_Interp *interp ) {
  ...
  Tcl_CreateObjCommand(interp, "vexpr", VecExpr_ObjCmd, (ClientData) 0, NULL);

  Tcl_PkgProvide(interp, "vexpr", VERSION);
