  if {[info exists ::tclmacbagscrolledframe::version]} { return }
  namespace eval ::tclmacbagscrolledframe \
  {
  # beginning of ::tclmacbagscrolledframe namespace definition
  ...
  # ------------------------------
  # Revised for TclMacBag collection by Peter Caffin
  # Pinstripe background, etc.
  # ==============================
    ...
    proc scrolledframe {w args} \
    {
      ...
      if {$ThemeNow == "aqua"} { frame $w -background systemModelessDialogBackgroundActive } else { ttk::frame $w } 
      # trap the reference
      rename $w ::tclmacbagscrolledframe::_$w
      # redirect to dispatch
      interp alias {} $w {} ::tclmacbagscrolledframe::dispatch $w
      # create scrollable internal frame
      if {$ThemeNow == "aqua"} { frame $w.scrolled -background systemModelessDialogBackgroundActive } else { ttk::frame $w.scrolled }
      # place it
      ...
      set ($w:filly)    0
      # configure
      if {$args != ""} { uplevel 1 ::tclmacbagscrolledframe::config $w $args }
      # bind <Configure>
    ...
    proc dispatch {w cmd args} \
    {
      ...
      switch -glob -- $cmd \
      {
        con*    { uplevel 1 [linsert $args 0 ::tclmacbagscrolledframe::config $w] }
        xvi*    { uplevel 1 [linsert $args 0 ::tclmacbagscrolledframe::xview  $w] }
        yvi*    { uplevel 1 [linsert $args 0 ::tclmacbagscrolledframe::yview  $w] }
        default { uplevel 1 [linsert $args 0 ::tclmacbagscrolledframe::_$w    $cmd] }
      }
    }
    ...
    proc config {w args} \
    {
      ...
      {
        # call frame config
        uplevel 1 [linsert $options 0 ::tclmacbagscrolledframe::_$w config]
      }
    }
  ...
  # end of ::tclmacbagscrolledframe namespace definition
  }

interp alias {} tclmacbag::scrolledframe {} ::tclmacbagscrolledframe::scrolledframe
