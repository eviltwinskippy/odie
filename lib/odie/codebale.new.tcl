...
proc ::codebale::c_detect_comment_or_macro {thisline resultvar priorcomment trace} {
  if {[string index $parseline 0] eq "#"} {
    if {[string index $thisline end] eq "\\"} {
      return 2
    } else {
      return 1
    }
...
# description:
# Returns <ol>
# <li> -1 C statement (no function)
# <li> 0 still parsing
# <li> 1 c function
# <li> 2 c function (still parsing)
###
proc ::codebale::c_detect_function {thisline resultvar priorcomment trace} {

  if {[string length $parseline]==0} {
    return -1
  }
  set match 0
  foreach exp {
    ...
    set n [string first "\{" $parseline]
    set match [regexp $exp [string range $parseline 0 $n]  all keywords funcname arglist]
    if { $match } break
  }
  if {!$match} {
  ...
  if {[string first "\{" $parseline] < 0} {
    puts "Waiting for bracket"
    return 2
  }
  if {![c_complete $parseline]} {
    puts "Waiting for complete"

    return 2
  }
  if {[string index [string trimright $parseline] end] eq "\;"} {
    return -1
  }
...
proc ::codebale::digest_csource {dat {trace 0}} {
  ...
  foreach rawline [split $dat \n] {
    ...
    if { $trace } {puts "$continueline $instatement $infunction $inparen $incomment [string length $priorcomment] | $line"}
    if {$incomment} {
      append thisline \n [string trim $line]
    ...
    set code [c_detect_comment_or_macro $parseline]
    switch $code {
      1 {
        set isfunct -1
      }
    ...
    set isfunct [c_detect_function $parseline result $priorcomment $trace]
    
    ###
    # Definition not complete yet
    if {$isfunct==2} {
      incr continueline
      continue
    }
...
proc ::codebale::headers_csourcefile file {  
  ...
  set dat [read $fin]
  close $fin
  if [catch [digest_csource $dat] result] {
    digest_csource $dat 1
    exit
  }
  set functions {}
  if [catch {
...
namespace eval ::codebale {
  ...
  parser_addpattern ooclass superclass     ::codebale::parse_oosuperclass
}
